...
{
	MidtermScene::RegisterComponentType<Camera>();
	MidtermScene::RegisterComponentType<RendererComponent>();
	MidtermScene::RegisterComponentType<BehaviourBinding>();
	MidtermScene::RegisterComponentType<PhysicsBody>();

	scene = MidtermScene::Create("GameScene");
	...
	Texture2D::sptr FireEnemy = Texture2D::LoadFromFile("image/Space.png");
	TextureCubeMap::sptr environmentMap = TextureCubeMap::LoadFromImages("image/skybox/ToonSky.jpg");

	Texture2D::sptr hand = Texture2D::LoadFromFile("image/corona_bk.png");
	...
	handMat->Shader = RenderingManager::NoOutline;
	...
	GameObject obj2 = scene->CreateEntity("ball");
	{
		obj2.get<Transform>().SetLocalPosition(2, 2, 2).SetLocalRotation(-90, 0, 0);
		//obj2.get<Transform>().SetParent(cameraObject);
		VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("model/sphear.obj");
